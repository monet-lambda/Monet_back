...
var WebMonitor = (function($, undefined) {
  ...
  var submitJob = function(taskName, args, doPoll) {
    ...
    jobRequest.fail(function(data, status) {
      jobPromise.reject(data, status);
    });
    return jobPromise;
  };
  ...
  var createTask = function(taskName, args) {
    ...
    jobPromise.done(function(job) {
      ...
      if (job["status"] === "failed") {
        ...
        if (result["success"] === false) {
          var message =
            "<p>The task could not complete successfully. It returned the error message " +
            "<code>" +
            result["message"] +
            "</code>." +
            "</p>";
    ...
    jobPromise.fail(function(data, status) {
      ...
      message =
        "<p>The server responded with the error code " +
        "<code>" +
        data.status +
        "</code>" +
        " and the message " +
