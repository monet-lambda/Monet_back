...
def update_references() -> Response:
    ...
    for idx, input_item in enumerate(inputs):
        ...
        if hist is None:
            logging.warning(f"Histogram '{histo_name}' not found in file for"
        ...
        if not isinstance(hist, (ROOT.TH1, ROOT.TH2)):
            logging.error(f"Object '{histo_name}' is not a histogram. "
                          "Skipping...")
...
def process_histogram(hist, run_number: int, scale: float):
    ...
    if cached_run_numbers != []:
        ...
        ref_array = np.load("/home/gkhreich/trial/"
                            f"reference_{histo_name}_{ref_rn}.npy")
        suma_array = np.load("/home/gkhreich/trial/"
                             f"reference_{histo_name}_{ref_rn}_suma.npy")
        x0_ = ref_array[0]
        e0_ = ref_array[1]
        ...
        suma_x0_filename =\
            "/home/gkhreich/trial/"\
            f"reference_{hist.GetName()}_{run_number}_suma.npy"
...
def construct_path(run_number: int, task_name: str) -> str:
    ...
    return f"/hist/Savesets/ByRun/{task_name}/{range_number}/{range_number2}/"\
