...
def only_numeric_strings_length(strings: list[str], length: int = 8) -> list[str]:
...
class SimProdFiles:
        ...
        if lock.is_locked:
            ...
            with lock:
                try:
                    ...
                    logging.error(f"Can't load Sim prod DB: {traceback.format_exc()}")
    ...
    def get_prod_info(
        self, request_id: int, event_type: int, filename: str = "1-GAUSSHIST.yml"
    ) -> any:
