...
from interfaces.rundb import get_latest_run_number, get_rundb_info
...
class lb_problemdb:
    ...
    def __init__(
        self, endpoint: str, target_app: str, audience: str, access_token: str
    ):
        """Initialize classe
    ...
    def get_token(self) -> None:
        ...
        r = requests.post(
            ...
            data={"grant_type": "client_credentials", "audience": self.target_app},
        )
        if r.ok:
            the_json = r.json()
    ...
    def search_issues(self, online: bool, run_number: int) -> list:
        ...
        if online:  # search opened problems
            the_args = {"open_or_closed_gte": datetime.datetime.now().astimezone()}
        elif run_number:
            the_args = {"run_number": run_number}
        try:
            r = requests.get(
                f"{self.url}api/search/", the_args, headers=self.auth_headers()
            )
        except Exception:
            return [
                {
    ...
    def create_issue(self, fields: dict[str, str]) -> bool:
        ...
        r = requests.post(
            f"{self.url}api/problems/", the_args, headers=self.auth_headers()
        )
...
class ProblemDBClient:
    ...
    def __init__(self, endpoint: str, project_name: str, username: str, key: str):
        """Initialize class
    ...
    def submit_problem(
        ...
        """
        if not run_number:
            run_number = get_latest_run_number(
    ...
    def get_run_status(self, online: bool, run_id: int) -> dict:
        ...
        result = {"status": "success" if problems == [] else "warning", "issues": []}
        for issue in problems:
            ...
            elif issue["severity"] == "QUERY":
                issue_status = "light"
            elif issue["severity"] == "UNKNOWN":
                issue_status = "secondary"
            ...
            result["issues"].append(
                {
                    ...
                    "status": issue_status,
                    "url": f"https://lbproblems.cern.ch/problemdb/problems/{issue['id']}",
                }
            )
        return result
