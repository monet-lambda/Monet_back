...
var online_run_number;window._refresh_enabled=false;var _refresh_rate=60000;function do_refresh(){if(!window._refresh_enabled)return;reload_page(true);}
function reload_page(refresh){var url="/online_dq";url+="?selected_page="+$(".jstree-container-ul").jstree("get_selected")[0];url+="&partition="+$("#partition-field").val();if(refresh)url+="&refresh=true";window.location.href=url;set_rundb_info();}
function enable_refresh(){window._refresh_enabled=true;var icon=$("#refresh-icon");icon.attr("class","glyphicon glyphicon-stop");$("#refresh-text").text("Stop refreshing");setTimeout(do_refresh,_refresh_rate);}
function disable_refresh(){window._refresh_enabled=false;var icon=$("#refresh-icon");$("#refresh-text").text("Start refreshing");icon.attr("class","glyphicon glyphicon-play");}
function set_rundb_info(){$.ajax({async:true,type:"GET",url:"/rundb?mode=online&partition="+$("#partition-field").val(),success:function(server_reply){if(!server_reply.success)return;var popover=$("#runInfo").data("bs.popover");popover.options.html=true;popover.options.sanitize=false;popover.options.content=server_reply.rundb_info;online_run_number=server_reply.rundb_info.match(/rundb-run-number\"\>(\d+)\</)[1];var popover_is_visible=$("#runInfo").data("bs.popover").tip().hasClass("in");if(popover_is_visible){$("#runInfo").popover("show");}}});}
$(function(){init_reference_state_button();$("#changeReferenceMode").click(function(){change_reference_mode();});$("#pageRefresh").click(function(){if(window._refresh_enabled)disable_refresh();else enable_refresh();});var refresh=getUrlParameter("refresh");if(refresh){enable_refresh();}
var select=document.getElementById("partition-field");select.addEventListener("change",function(){reload_page(false);});$('#runInfo').on('show.bs.popover',function(){$.ajax({async:false,type:"GET",url:"/rundb?mode=online&partition="+$("#partition-field").val(),success:function(server_reply){if(!server_reply.success)return;var popover=$("#runInfo").data("bs.popover");popover.options.html=true;popover.options.sanitize=false;popover.options.content=server_reply.rundb_info;online_run_number=server_reply.rundb_info.match(/rundb-run-number\"\>(\d+)\</)[1];}});})});function is_alarm_folder(node_id){if(node_id)return node_id.indexOf('alarm-subsys-folder')==0}
